...
Matrix3::Matrix3(const float* data_)
{
...
{
  Matrix3& self = *this;
  ErrorIf(Math::DebugIsZero(rhs), "Matrix3 - Division by zero.");
  self[0] /= rhs;
  self[1] /= rhs;
  self[2] /= rhs;
}
...
Matrix3 Matrix3::Concat(Mat3Param rhs) const
{
  ...
  ret.m00 = Dot(GetCross(0), rhs.Basis(0));
  ret.m01 = Dot(GetCross(0), rhs.Basis(1));
  ret.m02 = Dot(GetCross(0), rhs.Basis(2));

  ret.m10 = Dot(GetCross(1), rhs.Basis(0));
  ret.m11 = Dot(GetCross(1), rhs.Basis(1));
  ret.m12 = Dot(GetCross(1), rhs.Basis(2));

  ret.m20 = Dot(GetCross(2), rhs.Basis(0));
  ret.m21 = Dot(GetCross(2), rhs.Basis(1));
  ret.m22 = Dot(GetCross(2), rhs.Basis(2));
...
Matrix3::CrossVector Matrix3::GetCross(unsigned index) const
{
  const Matrix3& self = *this;
  ErrorIf(index > 2, "Matrix3 - Index out of range.");
#ifdef ColumnBasis
...
{
  float x = Dot(matrix.GetCross(0), vector);
  float y = Dot(matrix.GetCross(1), vector);
  float z = Dot(matrix.GetCross(2), vector);
...
Vector2 TransformDirection(Mat3Param matrix, Vec2Param normal)
{
