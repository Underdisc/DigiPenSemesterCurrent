...
std::vector<bool> knapsack_brute_force(std::vector<Item> const& items,
  Weight const& W )
{
  // best knapsack values
  std::vector<bool> best_fit(items.size(), false);
  int best_value = 0;
  // current knapsack values
  Weight current_weight;
  int current_value = 0;

  GreyCode grey_code(items.size());
  std::pair<bool, std::pair<bool, int> > next_ret;
  bool processing = true;
  // running through every possible permutation of items
  // with grey codes
  while(processing){
    // process next greycode
    next_ret = grey_code.Next();
    int item_changed = next_ret.second.second;
    if(next_ret.second.first){
      // item was added
      current_value += items[item_changed].GetValue();
      current_weight += items[item_changed].GetWeight();
    }
    else{
      // item was removed
      current_value -= items[item_changed].GetValue();
    ...
    // check to see if this is a new best fit
    if(current_weight <= W && current_value > best_value){
      best_fit = grey_code.GetCurrentGreyCode();
      best_value = current_value;
    }
    processing = next_ret.first;
  }
  // done
  return best_fit;
}
