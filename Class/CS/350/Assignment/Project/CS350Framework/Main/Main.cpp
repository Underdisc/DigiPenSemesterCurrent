...
bool CheckForUnitTests(int argc, char *argv[])
{
  ...
  std::string defaultFileSuffix = FormatString("%d_%d_%s", assignmentNumber, testNumber, configuration);
  defaults["-file"] = FormatString("Test_%s.txt", defaultFileSuffix.c_str());
  defaults["-time"] = FormatString("Timings_%s.txt", defaultFileSuffix.c_str());

  CommandLine* commandLine = CommandLine::GetInstance();
  commandLine->Parse(argc, argv, defaults);
  ArgumentMap& args = commandLine->mParsedArguments;

  // Auto append all arguments to the default file name?
  //for(size_t i = 0; i < commandLine->mArgumentList.size(); ++i)
  //  defaultFileSuffix += commandLine->mArgumentList[i];

  defaults["-file"] = FormatString("Test_%s.txt", defaultFileSuffix.c_str());
  defaults["-time"] = FormatString("Timings_%s.txt", defaultFileSuffix.c_str());

  std::string outFilePath = defaults["-file"];
