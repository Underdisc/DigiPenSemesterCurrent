...
// Compute the barycentric coordinates of a point to a triangle. Returns false if the triangle is degenerate
// or the passed in point is outside the triangle. The given epsilon should be used to expand the triangle before
// checking if the point is outside the triangle (use it to expand the range of the barycentric coordinate checks).
...
// Classify a point with respect to a plane. This should return Inside, Outside, or Coplanar 
// (in the event the the point is within epsilon distance from the plane).
...
// Epsilon is used to check if the ray is close enough to parallel to the plane; in this case return false.
bool RayPlane(const Vector3& rayStart, const Vector3& rayDir,
              const Vector4& plane, float& t, float parallelCheckEpsilon = 0.0001f);
...
// Use the triExpansionEpsilon to expand the triangle before checking for containment (in the barycentric coordinates test).
// This becomes useful and necessary later to deal with edge cases in a mesh (e.g. in the bsp tree).
// The parrallelCheckEpsilon is used to check if the ray is close to parallel to the plane of the triangle
bool RayTriangle(const Vector3& rayStart, const Vector3& rayDir,
                 const Vector3& triP0, const Vector3& triP1, const Vector3& triP2,
                 float& t, float triExpansionEpsilon, float parallelCheckEpsilon = 0.0001f);

// Check if a ray hits a sphere. If the ray does hit the sphere then t should be filled out with
// the first time of impact. If the ray starts inside the sphere then t should be 0. Note: t should
// never be set to a negative value if this function returns true!
bool RaySphere(const Vector3& rayStart, const Vector3& rayDir,
               const Vector3& sphereCenter, float sphereRadius,
               float& t);

// Check if a ray hits an aabb. If the ray does hit the aabb then t should be filled out with the
// first time of impact. If the ray starts inside the aabb then t should be 0. Note: t should never
// be set to a negative value if this function returns true! Do not call RayPlane here (it's inefficient)!
bool RayAabb(const Vector3& rayStart, const Vector3& rayDir,
             const Vector3& aabbMin, const Vector3& aabbMax, float& t, float parallelCheckEpsilon = 0.0001f);
...
// Classify an aabb against a plane. This should return Overlaps if the aabb hits the plane. Do not check all
// 8 points of the aabb against the plane! You must use one of the optimized methods described in class:
// the 2 point method or the radius method. No check for Coplanar is needed.
...
// This should also be used as the first axis you test so that temporal coherence can be used for rejection
// with only 1 plane test the next frame.
//--------------------------------------------------------------------------------------------------------------------

// Classify a triangle against a frustum. The given epsilon should be used for PlaneTriangle.
