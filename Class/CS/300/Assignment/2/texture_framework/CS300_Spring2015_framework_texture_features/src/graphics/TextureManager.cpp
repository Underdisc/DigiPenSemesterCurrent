...
namespace Graphics
{
  ...
  {
    // initialize slotBindings to false (none bound by default)
    std::memset(slotBindings_, false, sizeof(slotBindings_));
  }

  TextureManager::~TextureManager()
  {
    // deletes all textures, thereby calling their destructors and freeing any
    ...
    TextureType type, std::string const &textureFilePath)
  {
    ...
    else // new texture: insert(std::pair<TextureType, ...>(type, texture))
      return textures_.emplace(type, texture).first->second; // and return it
  }
