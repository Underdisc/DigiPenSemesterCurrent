...
namespace Math
{
  ...
  float& Vector3::operator[](unsigned index)
  {
    ...
    assert(index > 2, "Math::Vector3 - Subscript out of range.");
  ...
  {
    // changed to assert - ctd
    assert(vect == NULL, "Vector3 - Null pointer passed for vector.");
  ...
  {
    // changed to assert - ctd
    assert(vec == NULL, "Vector3 - Null pointer passed for vector.");
    vec->ZeroOut();
  }
